<%-- 
    Document   : delete_wrongly_encoded_asset
    Created on : 04 16, 23, 10:08:36 PM
    Author     : ccslearner
--%>
    ...
    <body>
        <% 
            ...
            PreparedStatement pstmt  = conn.prepareStatement("SELECT * FROM assets " +
                                                             "WHERE asset_id NOT IN (SELECT asset_id FROM asset_transactions) AND " +
                                                             "      asset_id NOT IN (SELECT asset_id FROM donated_assets);");
        ...
        %>
        <h1>Delete Wrongly Encoded Asset</h1>
        <p>Select an asset to delete below. Assets already used by activities cannot be deleted anymore.</p>
        
            <% while(rst.next()) { %>
            <a href="delete_wrongly_encoded_asset2.jsp?asset_id=<%= rst.getInt("asset_id") %>"> 
